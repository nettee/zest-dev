...
function deploySkills() {
  ...
  // Copy to Cursor
  const cursorTarget = path.join(process.cwd(), '.cursor/skills/zest-dev');
  fs.mkdirSync(cursorTarget, { recursive: true });
  copyDirectoryRecursive(sourceDir, cursorTarget);
  result.cursor.push('zest-dev/');

  // Copy to OpenCode
  const opencodeTarget = path.join(process.cwd(), '.opencode/skills/zest-dev');
  fs.mkdirSync(opencodeTarget, { recursive: true });
  copyDirectoryRecursive(sourceDir, opencodeTarget);
  result.opencode.push('zest-dev/');
